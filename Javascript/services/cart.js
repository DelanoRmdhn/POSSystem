...
export const calculateTax = function () {
  let currentTax = 0;
  for (let i = 0; i < state.cart.length; i++) {
    currentTax += state.cart[i].price * state.cart[i].quantity * 0.1; // 10% tax
  }
...
export const calculateTotal = function () {
  ...
  state.total = state.subtotal - state.discountAmount + state.tax;

  grandTotal.textContent = `Rp ${state.total}`;
};

export const runServices = function () {
  calculateSubtotal();
  calculateTax();
  calculateDiscount();
  calculateTotal();
